...
async function addHelp(data, openid) {
  ...
  if (data.type === 'express' || data.type === 'other') {
    if (!isValidAmount(data.reward) || Number(data.reward) > 10000) {
      return { code: -1, msg: '酬金必须大于 0、最多两位小数且不超过 1 万' }
    }
  }
...
async function updateHelp(data, openid) {
  ...
  if (updateData.reward !== undefined) {
    if (!isValidAmount(updateData.reward) || Number(updateData.reward) > 10000) {
      return { code: -1, msg: '酬金必须大于 0、最多两位小数且不超过 1 万' }
    }
...
async function acceptExpress(data, openid) {
  ...
  // 获取接单者信息
  const [acceptorInfo, publisherInfo] = await Promise.all([
    db.collection('users')
      .where({ openid })
      .field({ nickName: true })
      .get(),
    db.collection('users')
      .where({ openid: item.data.openid })
      .field({ nickName: true })
      .get()
  ])
